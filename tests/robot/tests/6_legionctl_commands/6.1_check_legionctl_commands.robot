...
Scale. Nonexistent model service
    ...
    ${res}=  Shell  legionctl --verbose scale this-model-does-not-exsit 1 --edi ${HOST_PROTOCOL}://edi-${MODEL_TEST_ENCLAVE}.${HOST_BASE_DOMAIN}
...
Scale. Nonexistent version of model service
    ...
    [Tags]  edi  cli  enclave
    ${res}=  Shell  legionctl --verbose scale ${TEST_COMMAND_MODEL_ID} 1 --model-version this-version-does-not-exsit --edi ${HOST_PROTOCOL}://edi-${MODEL_TEST_ENCLAVE}.${HOST_BASE_DOMAIN}
             Should not be equal  ${res.rc}  ${0}
             Should contain       ${res.stderr}  No one model can be found
...
Scale. Scale to zero replicas
    ...
    [Tags]  edi  cli  enclave
    ${res}=  Shell  legionctl --verbose scale ${TEST_COMMAND_MODEL_ID} 0 --edi ${HOST_PROTOCOL}://edi-${MODEL_TEST_ENCLAVE}.${HOST_BASE_DOMAIN}
             Should not be equal  ${res.rc}  ${0}
...
Scale. Scale to negative number of replicas
    ...
    [Tags]  edi  cli  enclave
    ${res}=  Shell  legionctl --verbose scale ${TEST_COMMAND_MODEL_ID} -3 --edi ${HOST_PROTOCOL}://edi-${MODEL_TEST_ENCLAVE}.${HOST_BASE_DOMAIN}
             Should not be equal  ${res.rc}  ${0}
...
Scale. Zero timeout parameter
    ...
    [Tags]  edi  cli  enclave
    ${res}=  Shell  legionctl --verbose scale ${TEST_COMMAND_MODEL_ID} 1 --timeout=0 --edi ${HOST_PROTOCOL}://edi-${MODEL_TEST_ENCLAVE}.${HOST_BASE_DOMAIN}
             Should not be equal  ${res.rc}  ${0}
...
Scale. Negative timeout parameter
    ...
    [Tags]  edi  cli  enclave
    ${res}=  Shell  legionctl --verbose scale ${TEST_COMMAND_MODEL_ID} 1 --timeout=-500 --edi ${HOST_PROTOCOL}://edi-${MODEL_TEST_ENCLAVE}.${HOST_BASE_DOMAIN}
             Should not be equal  ${res.rc}  ${0}
             Should contain       ${res.stderr}  should be positive integer
...
Scale. Negative timeout with no-wait parameter
    ...
    [Tags]  edi  cli  enclave
    ${res}=  Shell  legionctl --verbose scale ${TEST_COMMAND_MODEL_ID} 1 --no-wait --timeout=-500 --edi ${HOST_PROTOCOL}://edi-${MODEL_TEST_ENCLAVE}.${HOST_BASE_DOMAIN}
             Should be equal  ${res.rc}  ${0}

Scale. Missed the host parameter
    [Documentation]  The scale command must fail if it does not contain an edi host
    [Tags]  edi  cli  enclave
    ${res}=  Shell  legionctl scale ${TEST_COMMAND_MODEL_ID} 2
             Should not be equal  ${res.rc}  ${0}
...
Undeploy. Nonexistent model service
    ...
    [Tags]  edi  cli  enclave
    ${res}=  Shell  legionctl --verbose undeploy this-model-does-not-exsit --edi ${HOST_PROTOCOL}://edi-${MODEL_TEST_ENCLAVE}.${HOST_BASE_DOMAIN}
             Should not be equal  ${res.rc}  ${0}
             Should contain       ${res.stderr}  No one model can be found
...
Undeploy. Nonexistent version of model service
    ...
    [Tags]  edi  cli  enclave
    ${res}=  Shell  legionctl --verbose undeploy ${TEST_COMMAND_MODEL_ID} --model-version this-version-does-not-exsit --edi ${HOST_PROTOCOL}://edi-${MODEL_TEST_ENCLAVE}.${HOST_BASE_DOMAIN}
             Should not be equal  ${res.rc}  ${0}
             Should contain       ${res.stderr}  No one model can be found
...
Undeploy. Nonexistent model service with ignore-not-found parameter
    ...
    [Tags]  edi  cli  enclave
    ${res}=  Shell  legionctl --verbose undeploy this-model-does-not-exsit --ignore-not-found --edi ${HOST_PROTOCOL}://edi-${MODEL_TEST_ENCLAVE}.${HOST_BASE_DOMAIN}
             Should be equal  ${res.rc}  ${0}
...
Undeploy. Negative grace period parameter
    ...
    [Setup]  Run EDI deploy and check model started  ${MODEL_TEST_ENCLAVE}  ${TEST_MODEL_IMAGE_5}  ${TEST_COMMAND_MODEL_ID}  ${TEST_MODEL_5_VERSION}
    [Teardown]  Run EDI undeploy without version  ${MODEL_TEST_ENCLAVE}  ${TEST_COMMAND_MODEL_ID}
    ${res}=  Shell  legionctl --verbose undeploy ${TEST_COMMAND_MODEL_ID} --grace-period=-5 --edi ${HOST_PROTOCOL}://edi-${MODEL_TEST_ENCLAVE}.${HOST_BASE_DOMAIN}
             Should be equal  ${res.rc}  ${0}
...
Undeploy. Zero timeout parameter
    ...
    [Setup]  Run EDI deploy and check model started  ${MODEL_TEST_ENCLAVE}  ${TEST_MODEL_IMAGE_5}  ${TEST_COMMAND_MODEL_ID}  ${TEST_MODEL_5_VERSION}
    [Teardown]  Run EDI undeploy without version  ${MODEL_TEST_ENCLAVE}  ${TEST_COMMAND_MODEL_ID}
    ${res}=  Shell  legionctl --verbose undeploy ${TEST_COMMAND_MODEL_ID} --timeout=0 --edi ${HOST_PROTOCOL}://edi-${MODEL_TEST_ENCLAVE}.${HOST_BASE_DOMAIN}
             Should not be equal  ${res.rc}  ${0}
             Should contain       ${res.stderr}  should be positive integer
...
Undeploy. Negative timeout parameter
    ...
    [Setup]  Run EDI deploy and check model started  ${MODEL_TEST_ENCLAVE}  ${TEST_MODEL_IMAGE_5}  ${TEST_COMMAND_MODEL_ID}  ${TEST_MODEL_5_VERSION}
    [Teardown]  Run EDI undeploy without version  ${MODEL_TEST_ENCLAVE}  ${TEST_COMMAND_MODEL_ID}
    ${res}=  Shell  legionctl --verbose undeploy ${TEST_COMMAND_MODEL_ID} --timeout=-500 --edi ${HOST_PROTOCOL}://edi-${MODEL_TEST_ENCLAVE}.${HOST_BASE_DOMAIN}
             Should not be equal  ${res.rc}  ${0}
             Should contain       ${res.stderr}  should be positive integer
...
Undeploy. Negative timeout with no-wait parameter
    ...
    [Teardown]  Run EDI undeploy without version  ${MODEL_TEST_ENCLAVE}  ${TEST_COMMAND_MODEL_ID}
    [Tags]  edi  cli  enclave
    ${res}=  Shell  legionctl --verbose undeploy ${TEST_COMMAND_MODEL_ID} --no-wait --timeout=-500 --edi ${HOST_PROTOCOL}://edi-${MODEL_TEST_ENCLAVE}.${HOST_BASE_DOMAIN}
             Should be equal  ${res.rc}  ${0}

Undeploy. Missed the host parameter
    [Documentation]  The undeploy command must fail if it does not contain an edi host
    [Tags]  edi  cli  enclave
    ${res}=  Shell  legionctl undeploy ${TEST_COMMAND_MODEL_ID}
             Should not be equal  ${res.rc}  ${0}
             Should contain       ${res.stderr}  Failed to connect
...
Deploy. Nonexistent image
    ...
    [Tags]  edi  cli  enclave
    ${res}=  Shell  legionctl --verbose deploy nexus-local.cc.epm.kharlamov.biz:443/legion/this-image-does-not:exsit --edi ${HOST_PROTOCOL}://edi-${MODEL_TEST_ENCLAVE}.${HOST_BASE_DOMAIN}
             Should not be equal  ${res.rc}  ${0}
...
Deploy. Negative liveness timeout
    ...
    ${res}=  Shell  legionctl --verbose deploy ${TEST_MODEL_IMAGE_5} --livenesstimeout -3 --edi ${HOST_PROTOCOL}://edi-${MODEL_TEST_ENCLAVE}.${HOST_BASE_DOMAIN}
             Should not be equal  ${res.rc}  ${0}
...
Deploy. Negative readiness timeout
    ...
    [Tags]  edi  cli  enclave
    [Setup]  Run Keyword And Ignore Error  Run EDI undeploy without version  ${MODEL_TEST_ENCLAVE}  ${TEST_COMMAND_MODEL_ID}
    ${res}=  Shell  legionctl --verbose deploy ${TEST_MODEL_IMAGE_5} --readinesstimeout -3 --edi ${HOST_PROTOCOL}://edi-${MODEL_TEST_ENCLAVE}.${HOST_BASE_DOMAIN}
             Should not be equal  ${res.rc}  ${0}
             should contain  ${res.stderr}  must be greater than or equal to 0
...
Deploy. Zero replicas
    ...
    [Tags]  edi  cli  enclave
    [Setup]  Run Keyword And Ignore Error  Run EDI undeploy without version  ${MODEL_TEST_ENCLAVE}  ${TEST_COMMAND_MODEL_ID}
    ${res}=  Shell  legionctl --verbose deploy ${TEST_MODEL_IMAGE_5} --scale 0 --edi ${HOST_PROTOCOL}://edi-${MODEL_TEST_ENCLAVE}.${HOST_BASE_DOMAIN}
             Should not be equal  ${res.rc}  ${0}
             Should contain       ${res.stderr}  should be greater then 0
...
Deploy. Negative number of replicas
    ...
    [Tags]  edi  cli  enclave
    [Setup]  Run Keyword And Ignore Error  Run EDI undeploy without version  ${MODEL_TEST_ENCLAVE}  ${TEST_COMMAND_MODEL_ID}
    ${res}=  Shell  legionctl --verbose deploy ${TEST_MODEL_IMAGE_5} --scale -3 --edi ${HOST_PROTOCOL}://edi-${MODEL_TEST_ENCLAVE}.${HOST_BASE_DOMAIN}
             Should not be equal  ${res.rc}  ${0}
             Should contain       ${res.stderr}  should be greater then 0
...
Deploy. Zero timeout parameter
    ...
    [Tags]  edi  cli  enclave
    [Setup]  Run Keyword And Ignore Error  Run EDI undeploy without version  ${MODEL_TEST_ENCLAVE}  ${TEST_COMMAND_MODEL_ID}
    ${res}=  Shell  legionctl --verbose deploy ${TEST_MODEL_IMAGE_5} --timeout=0 --edi ${HOST_PROTOCOL}://edi-${MODEL_TEST_ENCLAVE}.${HOST_BASE_DOMAIN}
             Should not be equal  ${res.rc}  ${0}
             Should contain       ${res.stderr}  should be positive integer
...
Deploy. Negative timeout parameter
    ...
    [Tags]  edi  cli  enclave
    [Setup]  Run Keyword And Ignore Error  Run EDI undeploy without version  ${MODEL_TEST_ENCLAVE}  ${TEST_COMMAND_MODEL_ID}
    ${res}=  Shell  legionctl --verbose deploy ${TEST_MODEL_IMAGE_5} --timeout=-500 --edi ${HOST_PROTOCOL}://edi-${MODEL_TEST_ENCLAVE}.${HOST_BASE_DOMAIN}
             Should not be equal  ${res.rc}  ${0}
             Should contain       ${res.stderr}  should be positive integer
...
Deploy. Negative timeout with no-wait parameter
    ...
    [Tags]  edi  cli  enclave
    [Setup]  Run Keyword And Ignore Error  Run EDI undeploy without version  ${MODEL_TEST_ENCLAVE}  ${TEST_COMMAND_MODEL_ID}
    ${res}=  Shell  legionctl --verbose deploy ${TEST_MODEL_IMAGE_5} --no-wait --timeout=-500 --edi ${HOST_PROTOCOL}://edi-${MODEL_TEST_ENCLAVE}.${HOST_BASE_DOMAIN}
             Should be equal  ${res.rc}  ${0}

Deploy. Missed the host parameter
    [Documentation]  The deploy command must fail if it does not contain an edi host
    [Tags]  edi  cli  enclave
    ${res}=  Shell  legionctl deploy ${TEST_MODEL_IMAGE_5}
             Should not be equal  ${res.rc}  ${0}
             Should contain       ${res.stderr}  Failed to connect
