...
from legion.containers.docker import build_docker_client, find_host_model_port
...
class LocalEdgeClient(EdgeClient):
    ...
    def invoke_model_api(self, model_id: str, model_version: str, payload: typing.Dict[str, typing.Any],
        ...
        containers = client.containers.list(filters={
            'label': [f'{legion.k8s.definitions.LEGION_COMPONENT_LABEL}.model_id={model_id}',
                      f'{legion.k8s.definitions.LEGION_COMPONENT_LABEL}.model_version={model_version}']
        })
...
def build_client(args: argparse.Namespace = None) -> EdgeClient:
    ...
    host = host or legion.config.MODEL_SERVER_URL
    token = token or legion.config.get_config_file_variable(model_config_prefix(args.model_id, args.model_version),
                                                            section=legion.config.MODEL_JWT_TOKEN_SECTION)
