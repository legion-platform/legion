...
func Add(mgr manager.Manager) error {
	svr, err := webhook.NewServer("webhook-server", mgr, webhook.ServerOptions{
		...
		BootstrapOptions: &webhook.BootstrapOptions{
			MutatingWebhookConfigName:   viper.GetString(legion.MutatingWebhookName),
			ValidatingWebhookConfigName: viper.GetString(legion.ValidatingWebhookName),
			Secret: &types.NamespacedName{
				Namespace: viper.GetString(legion.Namespace),
				Name:      viper.GetString(legion.WebhookSecretName),
			},
