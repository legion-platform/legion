...
from legion.sdk.k8s.definitions import LEGION_COMPONENT_LABEL, LEGION_SYSTEM_LABEL, LEGION_API_SERVICE_PORT
from legion.sdk.k8s.definitions import LOAD_DATA_ITERATIONS, LOAD_DATA_TIMEOUT
from legion.sdk.k8s.exceptions import UnknownDeploymentForModelService, KubernetesOperationIsNotConfirmed
from legion.sdk.k8s.utils import normalize_k8s_name
...
class ModelService(Service):
    ...
    def _load_deployment_data(self):
        ...
        if not self._deployment:
            raise UnknownDeploymentForModelService(self.name)
    ...
    def check_service_is_deleted(self):
        ...
        """
        return not find_model_service(self.namespace, self.id, self.version)
    ...
    def check_deployment_is_deleted(self):
        ...
        """
        return not find_model_deployment(self.namespace, self.id, self.version)
    ...
    def metrics_name(self):
        ...
        """
        return '{}.{}'.format(normalize_name(self.id, dns_1035=True),
                              normalize_name(self.version, dns_1035=True))
...
def get_service(namespace='', component=''):
    ...
    return Service(result[0]) if result else None
