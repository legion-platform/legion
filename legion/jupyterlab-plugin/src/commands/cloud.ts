...
export function addCommands(options: IAddCloudCommandsOptions) {
    ...
    commands.addCommand(CommandIDs.newCloudTraining, {
        ...
        execute: args => {
            ...
            cloudDialogs.showCreateNewTrainingDialog(options.state.vcss, values)
                .then(({ value, button }) => {
                    if (button.accept) {
                        if (!value.isFinished) {
                            ...
                            options.api.cloud.createCloudTraining(value as models.ICloudTrainingRequest, options.state.credentials).then(() => {
                    ...
                    showDialog({
                        ...
                        if (button.accept) {
                            ...
                            options.api.cloud.removeCloudTraining({
                                name: name
                            }, options.state.credentials).then(() => {
                                splashScreen.dispose();
                                commands.execute(CommandIDs.refreshCloud, {});
                            }).catch(err => {
                                splashScreen.dispose();
                ...
                if (image) {
                    cloudDialogs.showCreateNewDeploymentDetails(
                        ...
                        if (button.accept) {
                            ...
                            }, options.state.credentials).then(_ => {
                                splashScreen.dispose();
                                commands.execute(CommandIDs.refreshCloud, {});
                            }).catch(err => {
                                splashScreen.dispose();
                    ...
                    dialogs.showChooseOrInputDialog(
                        ...
                        options.state.trainings.filter(training => training.status.modelImage.length > 0).map(training => {
                            return {
            ...
            try {
                ...
                if (newScale !== undefined) {
                    ...
                    dialogs.showChooseDialog(
                        ...
                        if (button.accept) {
                            const targetDeployment = options.state.deployments.find(deployment => deployment.name == value.value);
            ...
            try {
                ...
                if (name) {
                    ...
                    dialogs.showChooseDialog(
                        ...
                        if (button.accept) {
                            commands.execute(CommandIDs.removeCloudDeployment, { name: value.value });
                        }
                    })
                }
            } catch (err) {
        ...
        execute: args => {
            try {
                ...
                if (modelID) {
                    ...
                    }, options.state.credentials).then(response => {
                        splashScreen.dispose();
                        showDialog({
    ...
    commands.addCommand(CommandIDs.refreshCloud, {
        ...
        execute: () => {
            ...
            options.api.cloud.getCloudAllEntities(options.state.credentials).then(response => {
            ...
            try {
                ...
                if (name) {
                    ...
                    dialogs.showChooseDialog(
                        ...
                        if (button.accept) {
                            commands.execute(CommandIDs.openTrainingLogs, { name: value.value });
                        }
                    })
                }
            } catch (err) {
    ...
    commands.addCommand(CommandIDs.applyCloudResources, {
        ...
        execute: () => {
            ...
            let splashScreen = options.splash.show();
            options.api.cloud.applyFromFile({ path, removal: false }, options.state.credentials).then(results => {
                splashScreen.dispose();
    ...
    commands.addCommand(CommandIDs.removeCloudResources, {
        ...
        execute: () => {
            ...
            let splashScreen = options.splash.show();
            options.api.cloud.applyFromFile({ path, removal: true }, options.state.credentials).then(results => {
                splashScreen.dispose();
                dialogs.showApplyResultsDialog(results);
            }).catch(err => {
                splashScreen.dispose();
