...
> {
  constructor(props: ILocalWidgetViewNodeProps) {
    ...
    this.state = {
      ...
      buildingInProcess:
        props.dataState.buildStatus.started &&
        !props.dataState.buildStatus.finished,
  ...
  refresh = async () => {
    try {
      this.setState({
        ...
        buildingInProcess:
          this.props.dataState.buildStatus.started &&
          !this.props.dataState.buildStatus.finished,
  ...
  componentDidUpdate(
    _: ILocalWidgetViewNodeProps,
    oldState: ILocalWidgetViewNodeState
  ) {
    if (
      oldState.buildingInProcess !== this.state.buildingInProcess &&
  ...
  render() {
    return (
      <div className={style.widgetPane}>
        ...
        />
        {this.state.buildingInProcess ? (
          <div className={style.stripperLine}>BUILD IS IN PROCESS</div>
        ) : null}
        <ListingView
          title={'Builds'}
          topButton={
            <SmallButtonView
              ...
              onClick={() =>
                this.props.app.commands.execute(CommandIDs.newLocalBuild)
              }
            />
          }
          columns={[
        ...
        <ListingView
          ...
          topButton={
            <SmallButtonView
              ...
              iconClass={'jp-AddIcon'}
              onClick={() =>
                this.props.app.commands.execute(CommandIDs.newLocalDeployment)
              }
            />
          }
          columns={[
          ...
          items={this.state.localData.deployments.map(deployment => {
            return {
              ...
              onClick: () =>
                dialog
                  .showLocalDeploymentInformationDialog(deployment)
                  .then(({ button }) => {
                    if (button.label === dialog.REMOVE_DEPLOYMENT_LABEL) {
                      this.props.app.commands.execute(
                        CommandIDs.removeLocalDeployment,
