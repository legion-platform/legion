...
export class CloudTrainingLogsWidget extends BaseLegionWidget {
     ...
     */
    constructor(app: JupyterLab, api: ICloudApi, trainingName: string, state: IApiCloudState, options: IWidgetOptions) {
        super(options);
    ...
    refreshData() {
        ...
        this._startUpdating();
        this._api.getTrainingLogs({
            name: this._trainingName
        }, this._state.credentials).then(logs => {
            this._data = logs;
            this.component.refresh();
            this._finishUpdating();
        }).catch(err => {
