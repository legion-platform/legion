...
export namespace URLs {
    ...
    export const cloudTrainingLogsUrl = legionApiRootURL + '/cloud/trainings/:trainingName:/logs';
    ...
    export const cloudGetLocalFileInformationUrl = legionApiRootURL + '/cloud/local-file';
...
export interface ICloudApi {
    ...
    getTrainingLogs: (request: models.ICloudTrainingLogsRequest, credentials: ICloudCredentials) => Promise<models.ICloudTrainingLogsResponse>,
    ...
    // General
    applyFromFile: (request: models.IApplyFromFileRequest, credentials: ICloudCredentials) => Promise<models.IApplyFromFileResponse>,
}
...
export class CloudApi implements IApiGroup, ICloudApi {
    ...
    async getTrainingLogs(request: models.ICloudTrainingLogsRequest, credentials: ICloudCredentials): Promise<models.ICloudTrainingLogsResponse> {
        try {
            const url = URLs.cloudTrainingLogsUrl.replace(':trainingName:', request.name);
    ...
    async getLocalFileInformation(request: models.ILocalFileInformationRequest): Promise<models.ILocalFileInformationResponse> {
        try {
            let response = await httpRequest(URLs.cloudGetLocalFileInformationUrl, 'POST', request);
            if (response.status !== 200) {
                const data = await response.json();
                throw new ServerConnection.ResponseError(response, data.message);
            }
    ...
    // General
    async applyFromFile(request: models.IApplyFromFileRequest, credentials: ICloudCredentials): Promise<models.IApplyFromFileResponse> {
        try {
            let response = await httpRequest(URLs.cloudApplyFileUrl, 'POST', request, credentials);
            if (response.status !== 200) {
                const data = await response.json();
                throw new ServerConnection.ResponseError(response, data.message);
            }
