...
CONTAINER_IMAGE_ID_LINE = legion.containers.headers.STDERR_PREFIX + legion.containers.headers.IMAGE_ID_LOCAL
...
class TestLocalRun(unittest2.TestCase):
    ...
    def test_enter_exit_sandbox(self):
        with legion.utils.TemporaryFolder(change_cwd=True) as tempfs:
            ...
            with legion_test_utils.ManagedProcessContext([desired_path], shell=True) as context:
                ...
                container_id = legion.containers.docker.get_docker_container_id_from_cgroup_line(longest_line)
            ...
            self.assertTrue(legion.utils.ensure_function_succeed(check_container_removed, 10, 1, boolean_check=True),
    ...
    def test_build_math_model_python_script(self):
        with legion.utils.TemporaryFolder(change_cwd=True) as tempfs:
            ...
            legion.utils.copy_directory_contents(legion_test_utils.get_mocked_model_path('math_model'), tempfs.path)
