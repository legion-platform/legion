...
from legion.containers.docker import ModelBuildParameters
from legion.containers.headers import DOMAIN_MODEL_ID, DOMAIN_MODEL_VERSION, DOMAIN_CONTAINER_TYPE
...
class TestRemoteDockerBuild(unittest2.TestCase):
    ...
    def test_remote_build_and_push(self):
        with DockerBuildServer() as server, \
                patch('legion.containers.docker.commit_image', return_value='python:3.6'), \
    ...
    def test_failed_build(self):
        ...
        with DockerBuildServer() as server, \
                m_func('kubernetes.client.CoreV1Api.read_namespaced_pod', 'model'), \
                mock.patch('legion.containers.server._build_model_image', side_effect=Exception(error_msg)), \
                mock.patch('legion.k8s.utils.build_client', return_value=None):
    ...
    def test_request_timeout(self):
        with DockerBuildServer() as server, \
                m_func('kubernetes.client.CoreV1Api.read_namespaced_pod', 'model'), \
                mock.patch('legion.containers.server._build_model_image', side_effect=lambda _: sleep(15)), \
                mock.patch('legion.k8s.utils.build_client', return_value=None):
