...
def demo_abc_model_response(f):
    """
    Activate response library for a test method and create one 200 response for demo-abc-model 1.0
    :param f: name of function to moc
    :type f: function
    :return: function
    """
    @responses.activate
    def wrap(*args, **kwargs):
        responses.add(responses.GET, 'http://1.0.0.0:5000/api/model/demo-abc-model/1.0/info', status=200, json={
            'model_id': 'demo-abc-model',
            'model_version': '1.0',
...
class TestEDI(unittest2.TestCase):
        ...
        with EDITestServer() as edi:
            with m_func('kubernetes.client.CoreV1Api.list_namespaced_service', 'demo_abc_models_1_0_and_1_1'), \
                ...
                expected_result = {
                    ...
                    'model_api_info': {
                        'result': {
                            'endpoints': {'default': 'test'}, 'model_id': 'demo-abc-model', 'model_version': '1.0'
                        }
                    }
                }
                actual_result = models_info[0].as_dict()
                ...
                expected_result = {
                    ...
                    'model_api_ok': False,
                    'model_api_info': {
                        'exception': 'Connection refused: GET http://1.1.0.0:5000/api/model/demo-abc-model/1.1/info'
                    }
                }
                actual_result = models_info[1].as_dict()
                self.assertDictEqual(expected_result, actual_result)
    ...
    def test_edi_inspect_by_model_version(self):
        with EDITestServer() as edi:
            with m_func('kubernetes.client.CoreV1Api.list_namespaced_service', 'demo_abc_model_1_0'), \
                 m_func('kubernetes.client.ExtensionsV1beta1Api.list_namespaced_deployment', 'demo_abc_model_1_0'), \
                 mock.patch('legion.k8s.utils.build_client', return_value=None):
        ...
        with EDITestServer() as edi:
            with m_func('kubernetes.client.CoreV1Api.read_namespaced_service',
                 ...
                 m_func('kubernetes.client.ExtensionsV1beta1Api.read_namespaced_deployment',
                        'demo_abc_model_1_0'), \
                 m_func('kubernetes.client.CoreV1Api.create_namespaced_service', 'demo_abc_model_1_0'), \
                 mock.patch('legion.k8s.utils.build_client', return_value=None), \
                ...
                actual_result = deployments[0].as_dict()
                self.assertDictEqual(expected_result, actual_result)
    ...
    def test_edi_deploy_model_with_scale_1(self):
        with EDITestServer() as edi:
            with m_func('kubernetes.client.CoreV1Api.read_namespaced_service',
                 ...
                 m_func('kubernetes.client.ExtensionsV1beta1Api.create_namespaced_deployment',
                        'demo_abc_model_1_0') as cnd_mock, \
                    m_func('kubernetes.client.CoreV1Api.create_namespaced_service', 'demo_abc_model_1_0'), \
                    mock.patch('legion.k8s.utils.build_client', return_value=None), \
                    mock.patch('legion.k8s.enclave.Enclave.graphite_service', return_value=None), \
                    mock.patch('legion.k8s.utils.get_docker_image_labels', return_value=DOCKER_IMAGE_LABELS):
    ...
    def test_edi_deploy_model_with_scale_2(self):
        with EDITestServer() as edi:
            with m_func('kubernetes.client.CoreV1Api.read_namespaced_service',
                 ...
                 m_func('kubernetes.client.ExtensionsV1beta1Api.create_namespaced_deployment',
                        'demo_abc_model_1_0_scaled_to_2') as cnd_mock, \
                    m_func('kubernetes.client.CoreV1Api.create_namespaced_service', 'demo_abc_model_1_0'), \
                    mock.patch('legion.k8s.utils.build_client', return_value=None), \
                    mock.patch('legion.k8s.enclave.Enclave.graphite_service', return_value=None), \
                    mock.patch('legion.k8s.utils.get_docker_image_labels', return_value=DOCKER_IMAGE_LABELS):
    ...
    def test_negative_edi_deploy_scale_with_0(self):
        with EDITestServer() as edi:
            with mock.patch('legion.k8s.utils.build_client', return_value=None):
                with self.assertRaisesRegex(Exception, "Got error from server: 'Invalid scale parameter: "
        ...
        with EDITestServer() as edi:
            with m_func('kubernetes.client.CoreV1Api.list_namespaced_service', 'demo_abc_model_1_0'), \
                 ...
                 m_func('kubernetes.client.apis.apps_v1beta1_api.AppsV1beta1Api.delete_namespaced_deployment',
                        'demo_abc_model_1_0'), \
                 m_func('kubernetes.client.CoreV1Api.delete_namespaced_service',
                        ['demo_abc_model_1_0', 'demo_abc_model_1_0']), \
                 mock.patch('legion.k8s.utils.build_client', return_value=None), \
                 mock.patch('legion.k8s.enclave.Enclave.graphite_service', return_value=None):
    ...
    def test_negative_edi_undeploy_by_invalid_model_id(self):
        with EDITestServer() as edi:
            with m_func('kubernetes.client.CoreV1Api.list_namespaced_service', 'no_models'), \
                 mock.patch('legion.k8s.utils.build_client', return_value=None):
                with self.assertRaisesRegex(Exception, "Got error from server: 'No one model can be found'"):
    ...
    def test_negative_edi_scale_to_0(self):
        with EDITestServer() as edi:
            with m_func('kubernetes.client.CoreV1Api.list_namespaced_service', 'demo_abc_model_1_0'), \
                 m_func('kubernetes.client.ExtensionsV1beta1Api.list_namespaced_deployment', 'demo_abc_model_1_0'), \
                 mock.patch('legion.k8s.utils.build_client', return_value=None):
                with self.assertRaisesRegex(Exception, "Got error from server: 'Invalid scale parameter: should be "
                                                       "greater then 0'"):
    ...
    def test_edi_inspect_all_models_by_version(self):
        with EDITestServer() as edi:
            with m_func('kubernetes.client.CoreV1Api.list_namespaced_service', 'demo_abc_models_1_0_and_1_1'), \
                ...
                actual_result = deployments[1].as_dict()
                self.assertDictEqual(expected_result, actual_result)
        ...
        with EDITestServer() as edi:
            with m_func('kubernetes.client.CoreV1Api.list_namespaced_service', 'demo_abc_models_1_0_and_1_1'), \
                 m_func('kubernetes.client.ExtensionsV1beta1Api.list_namespaced_deployment',
                        'demo_abc_models_1_0_and_1_1'), \
                 mock.patch('legion.k8s.utils.build_client', return_value=None):
                with self.assertRaisesRegex(Exception, "Got error from server: 'Please specify version of model'"):
        ...
        with EDITestServer() as edi:
            with m_func('kubernetes.client.CoreV1Api.list_namespaced_service', 'demo_abc_models_1_0_and_1_1'), \
                 m_func('kubernetes.client.ExtensionsV1beta1Api.list_namespaced_deployment',
                        'demo_abc_models_1_0_and_1_1'), \
                 m_func('kubernetes.client.ExtensionsV1beta1Api.patch_namespaced_deployment',
                        ['demo_abc_model_1_0_scaled_to_2', 'demo_abc_model_1_1_scaled_to_2']) as pnd_mock, \
                    mock.patch('legion.k8s.utils.build_client', return_value=None), \
                    mock.patch('legion.k8s.enclave.Enclave.graphite_service', return_value=None):
