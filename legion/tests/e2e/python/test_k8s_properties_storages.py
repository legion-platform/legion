...
from legion.sdk.k8s.definitions import ENCLAVE_NAMESPACE_LABEL
from legion.sdk.k8s.enclave import Enclave
from legion.sdk.k8s.properties import K8SConfigMapStorage, K8SSecretStorage
...
class TestK8SPropertiesStorage(unittest2.TestCase):
    ...
    def setUpClass(cls):
        ...
        try:
            ...
            wait_until(check_cluster_is_active)
    ...
    def test_config_map_storage_watch(self):
        ...
        storage_to_write = K8SConfigMapStorage(storage_name, TEST_ENCLAVE_NAME)
        storage_to_write[key] = first_value
        storage_to_write.save()

        LOGGER.info('Retriving storage {!r}'.format(storage_name))
        storage_to_read = K8SConfigMapStorage.retrive(storage_name, TEST_ENCLAVE_NAME)
        ...
        with ContextThread(listener):
            ...
            self.assertTrue(wait_until(lambda: check_modification_event_recieved(events, {key: second_value}), 1, 5),
    ...
    def test_secret_storage_watch(self):
        ...
        LOGGER.info('Constructing initial {!r} storage with start value {!r}'
                    .format(storage_name, first_value))
        storage_to_write = K8SSecretStorage(storage_name, TEST_ENCLAVE_NAME)
        storage_to_write[key] = first_value
        storage_to_write.save()

        storage_to_read = K8SSecretStorage.retrive(storage_name, TEST_ENCLAVE_NAME)
        thread_ready = False
    ...
    def test_write_and_listing_config_map(self):
        ...
        self.assertIn(storage_name, K8SConfigMapStorage.list(TEST_ENCLAVE_NAME),
    ...
    def test_create_and_read_config_map_storage(self):
        ...
        items = K8SConfigMapStorage.list(TEST_ENCLAVE_NAME)
        self.assertIn(storage_name, items)

        LOGGER.info('Loading storage {!r}'.format(storage_name))
        storage_to_read = K8SConfigMapStorage(storage_name, TEST_ENCLAVE_NAME)
        storage_to_read.load()

        self.assertEqual(storage_to_read.get('int_var', cast=model.int32), 1233,
                         'Wrong casting to int')
        self.assertAlmostEqual(storage_to_read.get('float_var', cast=model.float32), 52.2354, 3,
                               'Wrong casting to float')
        self.assertEqual(storage_to_read.get('str_var', cast=model.string), 'Test string',
    ...
    def test_create_and_read_secret_storage(self):
        ...
        LOGGER.info('Listing storages')
        items = K8SSecretStorage.list(TEST_ENCLAVE_NAME)
        self.assertIn(storage_name, items, 'Storage {!r} has not presented in list of storages'
                      .format(storage_name))

        LOGGER.info('Loading storage {!r}'.format(storage_name))
        storage_to_read = K8SSecretStorage(storage_name, TEST_ENCLAVE_NAME)
        storage_to_read.load()
    ...
    def test_config_map_with_ttl(self):
        ...
        storage_to_read = K8SConfigMapStorage(storage_name, TEST_ENCLAVE_NAME, cache_ttl=ttl)
        storage_to_read.load()
