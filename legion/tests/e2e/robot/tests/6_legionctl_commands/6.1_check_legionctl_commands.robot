...
...                 Run EDI deploy and check model started  ${MODEL_TEST_ENCLAVE}  ${TEST_MODEL_IMAGE}  ${TEST_MODEL_ID}  ${TEST_MODEL_VERSION}  AND
...
Scale. Nonexistent version of model service
    ...
    ${res}=  Shell  legionctl --verbose scale ${TEST_MODEL_ID} 1 --model-version this-version-does-not-exsit --edi ${HOST_PROTOCOL}://edi-${MODEL_TEST_ENCLAVE}.${HOST_BASE_DOMAIN} --token "${DEX_TOKEN}"
             Should not be equal  ${res.rc}  ${0}
             Should contain       ${res.stderr}  No one model can be found

Scale. Scale to zero replicas
    [Documentation]  The scale command must fail if number of replicas is zero
    ${res}=  Shell  legionctl --verbose scale ${TEST_MODEL_ID} 0 --edi ${HOST_PROTOCOL}://edi-${MODEL_TEST_ENCLAVE}.${HOST_BASE_DOMAIN} --token "${DEX_TOKEN}"
             Should not be equal  ${res.rc}  ${0}
...
Scale. Scale to negative number of replicas
    ...
    ${res}=  Shell  legionctl --verbose scale ${TEST_MODEL_ID} -3 --edi ${HOST_PROTOCOL}://edi-${MODEL_TEST_ENCLAVE}.${HOST_BASE_DOMAIN} --token "${DEX_TOKEN}"
             Should not be equal  ${res.rc}  ${0}
...
Scale. Zero timeout parameter
    ...
    ${res}=  Shell  legionctl --verbose scale ${TEST_MODEL_ID} 1 --timeout=0 --edi ${HOST_PROTOCOL}://edi-${MODEL_TEST_ENCLAVE}.${HOST_BASE_DOMAIN} --token "${DEX_TOKEN}"
             Should not be equal  ${res.rc}  ${0}
...
Scale. Negative timeout parameter
    ...
    ${res}=  Shell  legionctl --verbose scale ${TEST_MODEL_ID} 1 --timeout=-500 --edi ${HOST_PROTOCOL}://edi-${MODEL_TEST_ENCLAVE}.${HOST_BASE_DOMAIN} --token "${DEX_TOKEN}"
             Should not be equal  ${res.rc}  ${0}
             Should contain       ${res.stderr}  should be positive integer
...
Scale. Negative timeout with no-wait parameter
    ...
    ${res}=  Shell  legionctl --verbose scale ${TEST_MODEL_ID} 1 --no-wait --timeout=-500 --edi ${HOST_PROTOCOL}://edi-${MODEL_TEST_ENCLAVE}.${HOST_BASE_DOMAIN} --token "${DEX_TOKEN}"
             Should be equal  ${res.rc}  ${0}

Scale. Missed the host parameter
    [Documentation]  The scale command must fail if it does not contain an edi host
    ${res}=  Shell  legionctl scale ${TEST_MODEL_ID} 2 --token "${DEX_TOKEN}"
             Should not be equal  ${res.rc}  ${0}
...
Undeploy. Nonexistent version of model service
    ...
    [Setup]  Run EDI deploy and check model started  ${MODEL_TEST_ENCLAVE}  ${TEST_MODEL_IMAGE}  ${TEST_MODEL_ID}  ${TEST_MODEL_VERSION}
    [Teardown]  Run EDI undeploy without version  ${MODEL_TEST_ENCLAVE}  ${TEST_MODEL_ID}
    ${res}=  Shell  legionctl --verbose undeploy ${TEST_MODEL_ID} --model-version this-version-does-not-exsit --edi ${HOST_PROTOCOL}://edi-${MODEL_TEST_ENCLAVE}.${HOST_BASE_DOMAIN} --token "${DEX_TOKEN}"
             Should not be equal  ${res.rc}  ${0}
             Should contain       ${res.stderr}  No one model can be found
...
Undeploy. Negative grace period parameter
    ...
    [Setup]  Run EDI deploy and check model started  ${MODEL_TEST_ENCLAVE}  ${TEST_MODEL_IMAGE}  ${TEST_MODEL_ID}  ${TEST_MODEL_VERSION}
    [Teardown]  Run EDI undeploy without version  ${MODEL_TEST_ENCLAVE}  ${TEST_MODEL_ID}
    ${res}=  Shell  legionctl --verbose undeploy ${TEST_MODEL_ID} --grace-period=-5 --edi ${HOST_PROTOCOL}://edi-${MODEL_TEST_ENCLAVE}.${HOST_BASE_DOMAIN} --token "${DEX_TOKEN}"
             Should be equal  ${res.rc}  ${0}
...
Undeploy. Zero timeout parameter
    ...
    [Setup]  Run EDI deploy and check model started  ${MODEL_TEST_ENCLAVE}  ${TEST_MODEL_IMAGE}  ${TEST_MODEL_ID}  ${TEST_MODEL_VERSION}
    [Teardown]  Run EDI undeploy without version  ${MODEL_TEST_ENCLAVE}  ${TEST_MODEL_ID}
    ${res}=  Shell  legionctl --verbose undeploy ${TEST_MODEL_ID} --timeout=0 --edi ${HOST_PROTOCOL}://edi-${MODEL_TEST_ENCLAVE}.${HOST_BASE_DOMAIN} --token "${DEX_TOKEN}"
             Should not be equal  ${res.rc}  ${0}
             Should contain       ${res.stderr}  should be positive integer
...
Undeploy. Negative timeout parameter
    ...
    [Setup]  Run EDI deploy and check model started  ${MODEL_TEST_ENCLAVE}  ${TEST_MODEL_IMAGE}  ${TEST_MODEL_ID}  ${TEST_MODEL_VERSION}
    [Teardown]  Run EDI undeploy without version  ${MODEL_TEST_ENCLAVE}  ${TEST_MODEL_ID}
    ${res}=  Shell  legionctl --verbose undeploy ${TEST_MODEL_ID} --timeout=-500 --edi ${HOST_PROTOCOL}://edi-${MODEL_TEST_ENCLAVE}.${HOST_BASE_DOMAIN} --token "${DEX_TOKEN}"
             Should not be equal  ${res.rc}  ${0}
             Should contain       ${res.stderr}  should be positive integer
...
Undeploy. Negative timeout with no-wait parameter
    ...
    [Setup]  Run EDI deploy and check model started  ${MODEL_TEST_ENCLAVE}  ${TEST_MODEL_IMAGE}  ${TEST_MODEL_ID}  ${TEST_MODEL_VERSION}
    [Teardown]  Run EDI undeploy without version  ${MODEL_TEST_ENCLAVE}  ${TEST_MODEL_ID}
    ${res}=  Shell  legionctl --verbose undeploy ${TEST_MODEL_ID} --no-wait --timeout=-500 --edi ${HOST_PROTOCOL}://edi-${MODEL_TEST_ENCLAVE}.${HOST_BASE_DOMAIN} --token "${DEX_TOKEN}"
             Should be equal  ${res.rc}  ${0}
...
Deploy. Negative liveness timeout
    ...
    [Setup]  Run Keyword And Ignore Error  Run EDI undeploy without version  ${MODEL_TEST_ENCLAVE}  ${TEST_MODEL_ID}
    ${res}=  Shell  legionctl --verbose deploy ${TEST_MODEL_IMAGE} --livenesstimeout -3 --edi ${HOST_PROTOCOL}://edi-${MODEL_TEST_ENCLAVE}.${HOST_BASE_DOMAIN} --token "${DEX_TOKEN}"
             Should not be equal  ${res.rc}  ${0}
...
Deploy. Negative readiness timeout
    ...
    [Setup]  Run Keyword And Ignore Error  Run EDI undeploy without version  ${MODEL_TEST_ENCLAVE}  ${TEST_MODEL_ID}
    ${res}=  Shell  legionctl --verbose deploy ${TEST_MODEL_IMAGE} --readinesstimeout -3 --edi ${HOST_PROTOCOL}://edi-${MODEL_TEST_ENCLAVE}.${HOST_BASE_DOMAIN} --token "${DEX_TOKEN}"
             Should not be equal  ${res.rc}  ${0}
             should contain  ${res.stderr}  must be greater than or equal to 0
...
Deploy. Zero replicas
    ...
    [Setup]  Run Keyword And Ignore Error  Run EDI undeploy without version  ${MODEL_TEST_ENCLAVE}  ${TEST_MODEL_ID}
    ${res}=  Shell  legionctl --verbose deploy ${TEST_MODEL_IMAGE} --scale 0 --edi ${HOST_PROTOCOL}://edi-${MODEL_TEST_ENCLAVE}.${HOST_BASE_DOMAIN} --token "${DEX_TOKEN}"
             Should not be equal  ${res.rc}  ${0}
             Should contain       ${res.stderr}  should be greater then 0
...
Deploy. Negative number of replicas
    ...
    [Setup]  Run Keyword And Ignore Error  Run EDI undeploy without version  ${MODEL_TEST_ENCLAVE}  ${TEST_MODEL_ID}
    ${res}=  Shell  legionctl --verbose deploy ${TEST_MODEL_IMAGE} --scale -3 --edi ${HOST_PROTOCOL}://edi-${MODEL_TEST_ENCLAVE}.${HOST_BASE_DOMAIN} --token "${DEX_TOKEN}"
             Should not be equal  ${res.rc}  ${0}
             Should contain       ${res.stderr}  should be greater then 0
...
Deploy. Zero timeout parameter
    ...
    [Setup]  Run Keyword And Ignore Error  Run EDI undeploy without version  ${MODEL_TEST_ENCLAVE}  ${TEST_MODEL_ID}
    ${res}=  Shell  legionctl --verbose deploy ${TEST_MODEL_IMAGE} --timeout=0 --edi ${HOST_PROTOCOL}://edi-${MODEL_TEST_ENCLAVE}.${HOST_BASE_DOMAIN} --token "${DEX_TOKEN}"
             Should not be equal  ${res.rc}  ${0}
             Should contain       ${res.stderr}  should be positive integer
...
Deploy. Negative timeout parameter
    ...
    [Setup]  Run Keyword And Ignore Error  Run EDI undeploy without version  ${MODEL_TEST_ENCLAVE}  ${TEST_MODEL_ID}
    ${res}=  Shell  legionctl --verbose deploy ${TEST_MODEL_IMAGE} --timeout=-500 --edi ${HOST_PROTOCOL}://edi-${MODEL_TEST_ENCLAVE}.${HOST_BASE_DOMAIN} --token "${DEX_TOKEN}"
             Should not be equal  ${res.rc}  ${0}
             Should contain       ${res.stderr}  should be positive integer
...
Deploy. Negative timeout with no-wait parameter
    [Documentation]  Ignore negative timeout parameter due to using no-wait parameter
    [Setup]  Run Keyword And Ignore Error  Run EDI undeploy without version  ${MODEL_TEST_ENCLAVE}  ${TEST_MODEL_ID}
    ${res}=  Shell  legionctl --verbose deploy ${TEST_MODEL_IMAGE} --no-wait --timeout=-500 --edi ${HOST_PROTOCOL}://edi-${MODEL_TEST_ENCLAVE}.${HOST_BASE_DOMAIN} --token "${DEX_TOKEN}"
             Should be equal  ${res.rc}  ${0}
...
Get token from EDI
    ...
    ${res} =  Shell  legionctl generate-token --edi ${HOST_PROTOCOL}://edi-${MODEL_TEST_ENCLAVE}.${HOST_BASE_DOMAIN} --model-id ${TEST_MODEL_ID} --model-version ${TEST_MODEL_VERSION} --token "${DEX_TOKEN}"
              Should be equal       ${res.rc}  ${0}
              Should not be empty   ${res.stdout}

    ${res} =  Shell  legionctl generate-token --edi ${HOST_PROTOCOL}://edi-${MODEL_TEST_ENCLAVE}.${HOST_BASE_DOMAIN} --model-id ${TEST_MODEL_ID} --token "${DEX_TOKEN}"
              Should not be equal   ${res.rc}  ${0}
              Should contain        ${res.stderr}  Requested field model_version is not set

    ${res} =  Shell  legionctl generate-token --edi ${HOST_PROTOCOL}://edi-${MODEL_TEST_ENCLAVE}.${HOST_BASE_DOMAIN} --model-version ${TEST_MODEL_VERSION} --token "${DEX_TOKEN}"
              Should not be equal   ${res.rc}  ${0}
              Should contain        ${res.stderr}  Requested field model_id is not set

    ${res} =  Shell  legionctl generate-token --edi ${HOST_PROTOCOL}://edi-${MODEL_TEST_ENCLAVE}.${HOST_BASE_DOMAIN} --model-version ${TEST_MODEL_VERSION} --token wrong-token
              Should not be equal   ${res.rc}  ${0}
              Should contain        ${res.stderr}  Credentials are not correct
...
Get token from EDI with expiration date set
    ...
    Log  ${expiration_date}
    ${res} =                Shell                     legionctl generate-token --edi ${HOST_PROTOCOL}://edi-${MODEL_TEST_ENCLAVE}.${HOST_BASE_DOMAIN} --expiration-date ${expiration_date} --model-id ${TEST_MODEL_ID} --model-version ${TEST_MODEL_VERSION} --token "${DEX_TOKEN}"
                            Should be equal           ${res.rc}  ${0}
                            Log  ${res.stdout}
...
Deploy fails when memory resource is incorect
    ...
    ${res}=  Shell  legionctl --verbose deploy ${TEST_MODEL_IMAGE} --memory wrong --edi ${HOST_PROTOCOL}://edi-${MODEL_TEST_ENCLAVE}.${HOST_BASE_DOMAIN} --token "${DEX_TOKEN}"
             Should not be equal  ${res.rc}  ${0}
...
Deploy fails when cpu resource is incorect
    ...
    ${res}=  Shell  legionctl --verbose deploy ${TEST_MODEL_IMAGE} --cpu wrong --edi ${HOST_PROTOCOL}://edi-${MODEL_TEST_ENCLAVE}.${HOST_BASE_DOMAIN} --token "${DEX_TOKEN}"
             Should not be equal  ${res.rc}  ${0}
...
Invoke. Empty model service url
    ...
    [Setup]  Run EDI deploy and check model started  ${MODEL_TEST_ENCLAVE}  ${TEST_MODEL_IMAGE}  ${TEST_MODEL_ID}  ${TEST_MODEL_VERSION}
    ${res}=  Shell  legionctl --verbose invoke --model-id ${TEST_MODEL_ID} --model-version ${TEST_MODEL_VERSION} -p a=1 -p b=2 --jwt "some_token"
             Should not be equal  ${res.rc}  ${0}
...
Invoke. Empty jwt
    ...
    ${res}=  Shell  legionctl --verbose invoke --model-id ${TEST_MODEL_ID} --model-version ${TEST_MODEL_VERSION} -p a=1 -p b=2 --model-server-url ${HOST_PROTOCOL}://edge-${MODEL_TEST_ENCLAVE}.${HOST_BASE_DOMAIN}
             Should not be equal  ${res.rc}  ${0}
...
Invoke. Wrong jwt
    ...
    [Setup]  Run EDI deploy and check model started  ${MODEL_TEST_ENCLAVE}  ${TEST_MODEL_IMAGE}  ${TEST_MODEL_ID}  ${TEST_MODEL_VERSION}
    ${res}=  Shell  legionctl --verbose invoke --model-id ${TEST_MODEL_ID} --model-version ${TEST_MODEL_VERSION} -p a=1 -p b=2 --model-server-url ${HOST_PROTOCOL}://edge-${MODEL_TEST_ENCLAVE}.${HOST_BASE_DOMAIN} --jwt wrong
             Should not be equal  ${res.rc}  ${0}
...
Invoke. Pass parameters explicitly
    ...
    ${res}=  Shell  legionctl --verbose invoke --model-id ${TEST_MODEL_ID} --model-version ${TEST_MODEL_VERSION} -p a=1 -p b=2 --model-server-url ${HOST_PROTOCOL}://edge-${MODEL_TEST_ENCLAVE}.${HOST_BASE_DOMAIN} --jwt "${JWT}"
             Should be equal  ${res.rc}  ${0}
...
Invoke. Pass parameters through config file
    ...
    [Setup]  Run EDI deploy and check model started  ${MODEL_TEST_ENCLAVE}  ${TEST_MODEL_IMAGE}  ${TEST_MODEL_ID}  ${TEST_MODEL_VERSION}
    Refresh security tokens
    ${res}=  Shell  legionctl --verbose invoke --model-id ${TEST_MODEL_ID} --model-version ${TEST_MODEL_VERSION} -p a=1 -p b=2
             Should be equal  ${res.rc}  ${0}
             Should contain   ${res.stdout}  42
...
Invoke. Fails with wrong model parameters
    ...
    [Setup]  Run EDI deploy and check model started  ${MODEL_TEST_ENCLAVE}  ${TEST_MODEL_IMAGE}  ${TEST_MODEL_ID}  ${TEST_MODEL_VERSION}
    Refresh security tokens
    ${res}=  Shell  legionctl --verbose invoke --model-id ${TEST_MODEL_ID} --model-version ${TEST_MODEL_VERSION} -p a=1
             Should not be equal  ${res.rc}  ${0}
...
Invoke. Pass model parameters using json
    ...
    [Setup]  Run EDI deploy and check model started  ${MODEL_TEST_ENCLAVE}  ${TEST_MODEL_IMAGE}  ${TEST_MODEL_ID}  ${TEST_MODEL_VERSION}
    Refresh security tokens
    ${res}=  Shell  legionctl --verbose invoke --model-id ${TEST_MODEL_ID} --model-version ${TEST_MODEL_VERSION} --json '{"a": 1, "b": 2}'
             Should be equal  ${res.rc}  ${0}
             Should contain   ${res.stdout}  42
...
Invoke. Pass model parameters using json and p
    ...
    [Setup]  Run EDI deploy and check model started  ${MODEL_TEST_ENCLAVE}  ${TEST_MODEL_IMAGE}  ${TEST_MODEL_ID}  ${TEST_MODEL_VERSION}
    Refresh security tokens
    ${res}=  Shell  legionctl --verbose invoke --model-id ${TEST_MODEL_ID} --model-version ${TEST_MODEL_VERSION} --json '{"a": 1}' -p b=2
             Should be equal  ${res.rc}  ${0}
             Should contain   ${res.stdout}  42
...
Invoke. Specify model endpoint
    ...
    [Setup]  Run EDI deploy and check model started  ${MODEL_TEST_ENCLAVE}  ${TEST_MODEL_IMAGE}  ${TEST_MODEL_ID}  ${TEST_MODEL_VERSION}
    Refresh security tokens
    ${res}=  Shell  legionctl --verbose invoke --model-id ${TEST_MODEL_ID} --model-version ${TEST_MODEL_VERSION} --endpoint=feedback -p str=aa -p copies=3
             Should be equal  ${res.rc}  ${0}
