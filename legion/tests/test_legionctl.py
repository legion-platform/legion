...
class TestLogin(unittest.TestCase):
    ...
    def test_override_default_config_path(self):
        with legion.utils.TemporaryFolder(change_cwd=True) as tempfs, \
            ...
            with patch.dict(os.environ, {'LEGION_CONFIG': str(new_config_path)}):
                self.assertEqual(legion.config.get_config_file_path(), new_config_path)
    ...
    def test_config_security_options(self):
        with legion.utils.TemporaryFolder(change_cwd=True) as tempfs, \
            ...
            self.assertEqual(legion.config.EDI_URL, edi)
            self.assertEqual(legion.config.EDI_TOKEN, token)
    ...
    def test_get_security_options(self):
        with legion.utils.TemporaryFolder(change_cwd=True) as tempfs, \
            ...
            security.login(build_model_args)
            legion.config.reinitialize_variables()
    ...
    def test_get_security_options_config_not_exist(self):
        with legion.utils.TemporaryFolder(change_cwd=True) as tempfs, \
            ...
            self.assertIsNone(legion.config.EDI_URL)
            self.assertIsNone(legion.config.EDI_TOKEN)
    ...
    def test_get_security_options_broken_config(self):
        with legion.utils.TemporaryFolder(change_cwd=True) as tempfs, \
            ...
            legion.config._INI_FILE_DEFAULT_CONFIG_PATH.parent.mkdir(parents=True, exist_ok=True)
            legion.config._INI_FILE_DEFAULT_CONFIG_PATH.touch(exist_ok=True)

            with legion.config._INI_FILE_DEFAULT_CONFIG_PATH.open('w') as f:
    ...
    def test_config_length(self):
        self.assertGreater(len(legion.config.ALL_VARIABLES), 0)
...
class TestSet(unittest.TestCase):
    ...
    def tearDown(self):
        if legion.config.ALL_VARIABLES.get(self.key):
            del legion.config.ALL_VARIABLES[self.key]

    def test_config_set_valid_param(self):
        with legion.utils.TemporaryFolder(change_cwd=True) as tempfs, \
                patch_config(build_legionctl_config_env(tempfs)):
            self.assertIsNone(legion.config.ALL_VARIABLES.get(self.key))
            legion.config.ConfigVariableDeclaration(self.key)
            config_set(self.args)
            self.assertEqual(legion.config.ALL_VARIABLES[self.key].name, self.key)
            self.assertEqual(getattr(legion.config, self.key), self.value)

    def test_config_set_not_configurable_manually(self):
        with legion.utils.TemporaryFolder(change_cwd=True) as tempfs, \
                patch_config(build_legionctl_config_env(tempfs)):
            self.assertIsNone(legion.config.ALL_VARIABLES.get(self.key))
            legion.config.ConfigVariableDeclaration(self.key, configurable_manually=False)
...
class TestGetAll(unittest.TestCase):
    ...
    def test_config_get_all_valid_param(self):
        ...
        legion.config.ConfigVariableDeclaration(configurable_key)
        ...
        legion.config.ConfigVariableDeclaration(secret_key, configurable_manually=True)
